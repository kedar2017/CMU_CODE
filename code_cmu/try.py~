...
def get_data():
	...
	global count
	new=[[],[],[]]
	for filename in glob.glob('/home/kedar/ccn/ml_task/left/0200_left.jpg'):
		...
		for i in range(2):
			...
			for j in range(len(img[i])):
				...
				for k in range(len(img[i][j])):
					if(not((j-2)>len(img[i])-1) and not((k-2)>len(img[i][j])-1)):
						train_ori_pixels[i]=train_ori_pixels[i]+[[img[i][j][k],img[i][j-1][k]],[img[i][j][k-1],img[i][j-1][k-1]]]
					if(count<2):
						if(not((j-2)>len(img[i])-1) and not((k-2)>len(img[i][j])-1)):
							test_ori_pixels[i]=test_ori_pixels[i]+[[img[i][j][k],img[i][j-1][k]],[img[i][j][k-1],img[i][j-1][k-1]]]
							valid_ori_pixels[i]=valid_ori_pixels[i]+[[img[i][j][k],img[i][j-1][k]],[img[i][j][k-1],img[i][j-1][k-1]]]
		if(count>3):
			break
	count=0
	lab=[]
	for filename in glob.glob('/home/kedar/ccn/ml_task/labeled/0200_left.png'):
		img=Image.open(filename)
		img=numpy.asarray(img,dtype='float64')
		count=count+1
		for i in range(len(img)):
			for j in range(len(img[i])):
				lab.append((img[i][j]+img[i][j+1]+img[i+1][j]+img[i+1][j+1])/4)
		train_lab=lab
		if(count<2):
			test_lab_pixels=lab
			valid_lab_pixels=lab
		if(count>3):
			break
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,dataset='mnist.pkl.gz',nkerns=[20, 50], batch_size=500):
	...
	layer0_input = x.reshape((batch_size, 3, 2, 2))
